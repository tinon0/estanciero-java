...
public class BotConservador extends Bot {
    private List<String> PreferenciasProvincias  = List.of("Formosa", "Rio Negro", "Salta");
    private  int propertiesPurchased;


    public BotConservador(String Name) {
        super(Name);
    ...
    public void buyProperty(List<Property> properties, Property property) {
        if(this.getMoney() >= property.getValue()){
            if(property.getPlayerOwner() == null) {
                ...
                if (isPriorityProvince || propertiesPurchased >= 4) {
    ...
    public void buyRailway(Railway railway, List<Railway> railwayList) {
        Printer.println("El bot "+ this.getName() + " ha decidido no comprar el ferrocaril");
    }
    ...
    public void addImprovements(Cell propiedadCaida, Bank banco) {
        if (propiedadCaida instanceof Property propCaida){
            ...
            if (count >= 2){
                if (propCaida.getPrecio_mejora() < ((30*this.getMoney())/100)){
                ...
                banco.getCantidadMejoras().put("Chacras", banco.getCantidadMejoras().get("Chacras") - 1);
                    ...
                    Printer.println(colorize("Supera el 30% de mi dinero, no lo compro!", Attribute.BLACK_TEXT(), Attribute.RED_BACK(), Attribute.ITALIC()));
                }
            }
            else{
